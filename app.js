...
const API = 'https://script.google.com/a/macros/greytip.com/s/AKfycbyTc6lKZ2H1OOqiqXp9imSBeHmWMT2ZYK5cuwifesAQrFtqsnHT4wYrc70vMFUAkM1boQ/exec';
...
function doLogin() {
  ...
  // Instantly load fallback data — no API call needed to login
  STATE.users       = FALLBACK_USERS;
  STATE.batches     = DEMO_BATCHES;
  ...
  if(!user){
    showLoginError('Email not found. Try: priya@greytip.com · manager1@greytip.com · ravi@greytip.com');
    return;
  }
  ...
  // Silently try Sheets in background after login
  loadSheetsBackground();
}

async function loadSheetsBackground() {
  try {
    const init = await Promise.race([
      apiGet({ action:'init' }),
      new Promise(r => setTimeout(() => r(null), 6000))
    ]);
    if(init && init.status === 'ok') {
      if(init.users && Object.keys(init.users).length > 0)
        STATE.users = { ...FALLBACK_USERS, ...init.users };
      if(init.batches && init.batches.length > 0)
      ...
      toast('Live data loaded from Sheets ✓','success');
    }
...
// Fallback users — used when Sheet is empty or unreachable
...
const DEMO_ASSIGNMENTS = [
  {AssignID:'AS1',Title:'Assignment 1 — Discovery Questions',BatchID:'A',BatchName:'Sales Team — Batch A',DueDate:'Mar 20',Status:'Closed',Submitted:14,TotalParticipants:14,UploadedBy:'Priya K.',UploadedOn:'Mar 15',DriveLink:''},
  {AssignID:'AS2',Title:'Assignment 2 — Objection Handling', BatchID:'A',BatchName:'Sales Team — Batch A',DueDate:'Apr 5', Status:'Open',  Submitted:7, TotalParticipants:14,UploadedBy:'Priya K.',UploadedOn:'Mar 28',DriveLink:''},
  {AssignID:'AS3',Title:'Case Study — greytHR Deal Scenario',BatchID:'C',BatchName:'Support — Batch C',   DueDate:'Apr 10',Status:'Open',  Submitted:4, TotalParticipants:18,UploadedBy:'Priya K.',UploadedOn:'Apr 1', DriveLink:''},
  {AssignID:'AS4',Title:'MEDDIC Role-Play Reflection',       BatchID:'D',BatchName:'Impl. — Batch D',    DueDate:'Apr 3', Status:'Closed',Submitted:14,TotalParticipants:14,UploadedBy:'Priya K.',UploadedOn:'Mar 30',DriveLink:''},
];

const DEMO_MATERIALS = [
  {Name:'MEDDIC Framework Reference Guide', BatchID:'D',BatchName:'Impl. Batch D',  Type:'Guide',  Size:'1.2 MB',UploadedOn:'Apr 1', DriveLink:''},
  {Name:'Enterprise Sales — Module 1 Deck',BatchID:'A',BatchName:'Sales Batch A',  Type:'Slides', Size:'3.4 MB',UploadedOn:'Mar 28',DriveLink:''},
  {Name:'Participant Handout — Session 2',  BatchID:'B',BatchName:'CS Batch B',     Type:'Handout',Size:'0.8 MB',UploadedOn:'Mar 25',DriveLink:''},
  {Name:'Case Study Discussion Template',   BatchID:'ALL',BatchName:'All Batches',  Type:'Template',Size:'0.5 MB',UploadedOn:'Mar 20',DriveLink:''},
  {Name:'Comm. Skills — Session Recording', BatchID:'C',BatchName:'Support Batch C',Type:'Video',  Size:'Link',  UploadedOn:'Apr 1', DriveLink:''},
];
